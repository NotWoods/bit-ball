...
--[[ --Basic Platform
local platformA = display.newImage("yellow_platform.png") 
platformA.x = 300; platformA.y = 150; platformA.rotation = 35
...
trampolineA.x = 20; trampolineA.y = 380; trampolineA.rotation = 20
...
local function replayPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_4", "fade") ------------------------------------------------------------------------------- X = Current Level
	end
end
...
local function nextPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_5", "fade") ------------------------------------------------------------------------------- X = Next Level
	end
end
...
local function onLocalCollision( self, event ) -- Triggers win when ball enters goal
    if ( event.phase == "began" ) and ( alreadyWon == false ) then
		...
		local wasAdded = level:storeIfHigher( "levelPack1", 5 ) --------------------------------------------------------------- X = Next Level 
		if wasAdded then
		level:save()
		end
		local scoreSave = ice:loadBox( "scoreSave" )
		local highscore = scoreSave:storeIfHigher( "level4", score ) ---------------------------------------------------------- X = Current Level
		if highscore then
		scoreSave:save()
		end
    end
