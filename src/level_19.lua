...
platformA.x = 275; platformA.y = 290; platformA.rotation = -30
...
trampolineA.x = 140; trampolineA.y = 370; trampolineA.rotation = 19
...
local function replayPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_19", "fade") ------------------------------------------------------------------------------- X = Current Level
	end
end
...
local function nextPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_20", "fade") ------------------------------------------------------------------------------- X = Next Level
	end
end
...
local function onLocalCollision( self, event ) -- Triggers win when ball enters goal
    if ( event.phase == "began" ) and ( alreadyWon == false ) then
		...
		local wasAdded = level:storeIfHigher( "levelPack1", 20 ) --------------------------------------------------------------- X = Next Level 
		if wasAdded then
		level:save()
		end
		local scoreSave = ice:loadBox( "scoreSave" )
		local highscore = scoreSave:storeIfHigher( "level19", score ) ---------------------------------------------------------- X = Current Level
		if highscore then
		scoreSave:save()
		end
    end
