...
---[[ --Basic Platform
local platformA = display.newImageRect("platform_white_small.png", 100, 29)
platformA.x = 200; platformA.y = 385; platformA.rotation = -10
...
trampolineA.x = 30; trampolineA.y = 390; trampolineA.rotation = 8
smallPlatShape = { -38,-5, 38,-5, 38,4, -38,4 }
...
trampolineB.x = 120; trampolineB.y = 80; trampolineB.rotation = 2
...
touchA.x = 129; touchA.y = 220; touchA.rotation = -50 --Correct Rotation is 60
...
local function replayPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_18", "fade") ------------------------------------------------------------------------------- X = Current Level
	end
end
...
local function nextPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_19", "fade") ------------------------------------------------------------------------------- X = Next Level
	end
end
...
local function onLocalCollision( self, event ) -- Triggers win when ball enters goal
    if ( event.phase == "began" ) and ( alreadyWon == false ) then
		...
		local wasAdded = level:storeIfHigher( "levelPack1", 19 ) --------------------------------------------------------------- X = Next Level 
		if wasAdded then
		level:save()
		end
		local scoreSave = ice:loadBox( "scoreSave" )
		local highscore = scoreSave:storeIfHigher( "level18", score ) ---------------------------------------------------------- X = Current Level
		if highscore then
		scoreSave:save()
		end
    end
