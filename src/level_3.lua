...
trampolineA.x = 20; trampolineA.y = 400; trampolineA.rotation = 10
physics.addBody( trampolineA, "static", { density = 1.0, friction = 0.3, bounce = 0.7 } )
...
physics.addBody( trampolineA, "static", 
	{ density = 1.0, friction = 0.3, bounce = 0.7, shape=smallPlatShape } 
)
localGroup:insert(trampolineA)
--]]
...
local gearA = display.newImage( "gear.png" )
gearA.x, gearA.y = 100,270
physics.addBody( gearA, "dynamic", { friction=1, bounce=.2, radius = 16 } )
localGroup:insert(gearA)
 
-- create and position the dynamic platform
local rotaterA = display.newImage( "gray_platform_small.png" )
rotaterA.x, rotaterA.y = 100, 270
...
local pivot = physics.newJoint(
    ...
    100, 270 -- physics world location to join at
)
 
 --[[ create a pivot to join the gear to the platform
local gearAjoint = physics.newJoint(
    "pivot", -- joint type
    rotaterA, gearA, -- objects to join together
...
local rotaterB = display.newImage("platform_gray_small.png")
rotaterB.x, rotaterB.y = 190,230
...
local pivotB = physics.newJoint(
    "pivot", -- joint type
    anchorA, rotaterB, -- objects to join together
...
local function replayPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_3", "fade") ------------------------------------------------------------------------------- X = Current Level
	end
end
...
local function nextPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_4", "fade") ------------------------------------------------------------------------------- X = Next Level
	end
end
...
local function onLocalCollision( self, event ) -- Triggers win when ball enters goal
    if ( event.phase == "began" ) and ( alreadyWon == false ) then
		...
		local wasAdded = level:storeIfHigher( "levelPack1", 4 ) --------------------------------------------------------------- X = Next Level 
		if wasAdded then
		level:save()
		end
		local scoreSave = ice:loadBox( "scoreSave" )
		local highscore = scoreSave:storeIfHigher( "level3", score ) ---------------------------------------------------------- X = Current Level
		if highscore then
		scoreSave:save()
		end
    end
