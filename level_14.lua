...
platformA.x = 160; platformA.y = 280; platformA.rotation = 15
...
trampolineA.x = 290; trampolineA.y = 300; trampolineA.rotation = -20
smallPlatShape = { -38,-5, 38,-5, 38,4, -38,4 }
...
local function replayPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_14", "fade") ------------------------------------------------------------------------------- X = Current Level
	end
end
...
local function nextPress ( event )
	if ( event.phase == "ended" ) then
		...
		scoreDisplay.isVisible = false
		alreadyWon = false
		director:changeScene("level_15", "fade") ------------------------------------------------------------------------------- X = Next Level
	end
end
...
local function onLocalCollision( self, event ) -- Triggers win when ball enters goal
    if ( event.phase == "began" ) and ( alreadyWon == false ) then
		...
		local wasAdded = level:storeIfHigher( "levelPack1", 15 ) --------------------------------------------------------------- X = Next Level 
		if wasAdded then
		level:save()
		end
		local scoreSave = ice:loadBox( "scoreSave" )
		local highscore = scoreSave:storeIfHigher( "level14", score ) ---------------------------------------------------------- X = Current Level
		if highscore then
		scoreSave:save()
		end
    end
